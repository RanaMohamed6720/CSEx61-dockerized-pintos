...
process_execute (const char *file_name)
{
    ...
    sema_down(&thread_current()->parent_child_sync); // initialized with 0, so it waits
...
start_process (void *file_name_)
{
  ...
  if(success){
    ...
    parent->child_success_creation = true; 
  }else parent->child_success_creation = false; 
...
process_exit (void) {
    ...
    // Free all opened files
    struct list* process_files = &thread_current()->file_list;
    for(struct list_elem* i = list_begin(process_files);i !=list_end(process_files) ; )
    {
      struct file_elem* file = list_entry(i, struct file_elem , elem);
      i=list_next(i);
      close(file->fd);
      free(file);
    }

    // Wake up all children(although it doesn't ruin any test)
    struct list* all_children = &thread_current()->child_list;
    ...
    // Wake up parent, if the cur has a child
    if (cur->parent != NULL && cur->parent->waiting_on == cur->tid)
    {  
      cur->parent->child_status = thread_current()->exit_status;
      cur->parent->waiting_on = NULL;
      sema_up(&cur->parent->wait_child);
    }
    char *save_ptr;
    // tokenize file name based on the delimiter " "
    const char *file_name = strtok_r (cur->name," ",&save_ptr);
...
static bool setup_stack(void **esp, char *file_name) {
    ...
    int i;
    // tokenize the file name again and push arguments onto the stack
    ...
    // align the stack pointer to a multiple of 4
